...
public class Sprite {
  ...
  public static enum Action {
    NONE, RUNNING_STEP, RUNNING_NO_STEP, JUMPING, TURNING, DYING, SQUASHED
  };
  ...
  BufferedImage[] sprites = new BufferedImage[NUM_IMAGES * 2];
  ...
  public Sprite(String image) {
    ...
    try {
      ...
      for (int i = 0; i < NUM_IMAGES; i++) {
        ...
        // TODO make use of these instead of flipping the image while the game is running!
        // Store reverse sprite sheet
        AffineTransform result = AffineTransform.getScaleInstance(-1.0, 1.0);
        result.translate(-sprites[i].getWidth(), 0);
        AffineTransformOp op = new AffineTransformOp(result, AffineTransformOp.TYPE_NEAREST_NEIGHBOR);
        sprites[i + NUM_IMAGES] = op.filter(sprites[i], null);
      }
    } catch (IOException e) {
      // TODO UH OH
      e.printStackTrace();
  ...
  public AffineTransform getTransform() {
    AffineTransform result = AffineTransform.getTranslateInstance(position.x,
        position.y);
