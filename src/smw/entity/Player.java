...
public class Player extends Rectangle2D.Float implements Drawable, Updatable{
  ...
  public static final int WIDTH = 22;
	...
	static final int WRAP_AROUND_FACTOR = (GameFrame.res_width - Player.WIDTH);
  ...
  List<Rectangle> deathCheck = new ArrayList<Rectangle>();
    ...
    //#############################################################
		...
		for(Player p : players){
			if(p.playerIndex != playerIndex){
				if(p.isAlive()){
				  ...
				  if(p.intersects(newX, y, Player.WIDTH, Player.HEIGHT)){
				    if(p.x > newX && (p.x - newX < 2* Player.WIDTH)){
	            newX = p.x -  Player.WIDTH - 1;
	          }
	          else{
	            newX = p.x +  Player.HEIGHT + 1;
	          }
	          
	          xCollide = true;
	          break;
				  }
				  else if((newX +  Player.WIDTH > GameFrame.res_width) && p.intersects(newX - GameFrame.res_width, y,  Player.WIDTH,  Player.HEIGHT)){
				    newX = p.x -  Player.HEIGHT ;
            
            xCollide = true;
            break;
				  }
				  else if((p.x  +  Player.WIDTH > GameFrame.res_width) && p.intersects(newX + GameFrame.res_width, y,  Player.WIDTH,  Player.HEIGHT)){
		...
		for(Player p : players){
			if(p.playerIndex != playerIndex){
				if(p.isAlive() && p.intersects(x, newY,  Player.WIDTH,  Player.HEIGHT)){
					if(!xCollide){
						if(p.getY() < newY){
	...
	public void draw(Graphics2D graphics, ImageObserver observer){
	  ...
	  if(warpingIn){
	    ...
	    graphics.drawImage(sprite.getImage(), (int)(x)- Sprite.offSetX, (int)(y) - Sprite.offSetY, observer);
	  }
